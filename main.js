...
// Calculate initial pixel spacing in world units here to pass to manager
panelManager.setSpacing(panelManager.initialConfig.gridSpacingPx);
...
document.getElementById('apply-settings-button').addEventListener('click', () => {
    ...
    const settings = {
        ...
        // Reset texture if file input is cleared but was previously used (advanced)
        // textureUrl: textureInput.files.length === 0 ? panelManager.getPanelById(panelId)?.frameTextureUrl : undefined
    };
    ...
    // Clear file input after applying (optional)
    // textureInput.value = '';
